	  ...
	  if ( (this->getElements().size() == atom.getElements().size()) && (this->getAtomOp() == atom.getAtomOp()))
		{
			...
			{
				//cout << atom.toString() << " " << this->toString() << "equal" << endl;
				return 0;
			}	
		}
	  return 1;
	...
	int containsFormula(Formula &F) //to be completed and performed
	{
		for (int i = 0; i < this->getSetFormulae().size(); i++)
		{
			...
			if (locAt != NULL && F.getAtom() != NULL)
			{
				...
				{   
					cout << "llllcontains" << locAt->toString() << " "<< F.getAtom()->toString() <<endl;
					return 0;
				}
			}
...
public:
	...
	void insertFormula(Formula& f)
	{
		...
		{
			setFormula.push_back(f); cout << "inserting " << f.toString() << endl;
		}
	};
...
int checkNPAtoms(Atom* atom, Node* node)
{
	Node* iterator = node;
	while (iterator != NULL)
	{
		...
		for (int i = 0; i < local.size(); i++)
		{
			for (int j = 0; j < iterator->getSetFormulae().size(); j++)
			{
				if (iterator->getSetFormulae().at(j).getAtom() != NULL)
				{				
				  if ((checkAtomClash(*atom, *(iterator->getSetFormulae().at(j).getAtom())) == 0))
					return 0;
				  if (atom->equals(*(iterator->getSetFormulae().at(j).getAtom())))
					  return -1;
			     }
		     }
		}
...
void chooseRule(Tableau &T, vector<Node*> &nodeSet, Formula &f)
{
	...
	for (int b = 0; b < nodeSet.size(); b++)
	{
		...
		int j = 0;
		for (; j < atomset.size(); j++)
		{
			int val = checkNPAtoms(atomset.at(j), nodeSet.at(b));
			if ( val == 0)//replace with a function that looks for positive and integere at the same time
			{
...
int main()
{
	...
	*/
	...
	Atom* atom1 = new Atom(1, { &h1,&x, &b1 });
	cout << "ss" << atom->equals(*atom1) << endl;
